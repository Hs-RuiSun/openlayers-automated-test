...
import com.inflectra.spiratest.addons.junitextension.commons.ScreenShot;
...
import com.inflectra.spiratest.addons.junitextension.soap.RemoteDocument;
...
public class SpiraTestExecute {
	...
	public static String url = "http://10.12.5.23/SpiraTeam";
	public static String userName = JsonData.spiraUser;
	public static String password = JsonData.spiraPass;
	public static int spiraProjectId = JsonData.projectID;

	/**
	 * Records a test run
	 * 
	 * @param testerUserId
	 *            The user id of the person who's running the test (null for logged
	 *            in user)
	 * @param testCaseId
	 ...
	 *            The name of the test as stored in JUnit
	 ...
	 */
	public static int recordTestRun(Integer testerUserId, int testCaseId, Integer releaseId, Integer testSetId,
			Date startDate, Date endDate, int executionStatusId, String runnerName, String runnerTestName,
		...
		try {
			...
			URL serviceUrl = new URL(url + WEB_SERVICE_SUFFIX);
			...
			// If one is NULL, simply set to the same as the other to avoid
			// having to add NULL checks in the subsequent code
			if (soap == null) {
				soap = soap1;
			} else {
				soap1 = soap;
			...
			try {
				success = soap.connectionAuthenticate2(userName, password, SPIRA_PLUG_IN_NAME);
			} catch (Exception e) {
				System.out.println(e);
			} /*
				 * catch (InaccessibleWSDLException ex) { // Try using the second binding soap =
				 * soap1; success = soap.connectionAuthenticate2(userName, password,
				 * SPIRA_PLUG_IN_NAME); } catch (ClientTransportException ex) { // Try using the
				 * second binding soap = soap1; success = soap.connectionAuthenticate2(userName,
				 * password, SPIRA_PLUG_IN_NAME); }
				 */
			if (!success) {
				// Display the error
				System.out.print(
			...
			success = soap.connectionConnectToProject(spiraProjectId);
			if (!success) {
				System.out.print("Unable to connect to SpiraTest project " + spiraProjectId
			...
			if (ScreenShot.scrFile != null) {
				byte[] attachmentData = ScreenShot.scrFile;
				RemoteDocument remoteDocument = new RemoteDocument();
				String filename = JsonData.title;
				remoteDocument.setFilenameOrUrl(CreateJAXBString("FilenameOrUrl", filename));
				String description = JsonData.title;
				remoteDocument.setDescription(CreateJAXBString("Description", description));
				remoteDocument.setUploadDate(convertDatesJava2Xml(startDate));
				remoteDocument.setSize(attachmentData.length);
			 ...
			 * catch
			 * (IImportExportConnectionAuthenticate2ServiceFaultMessageFaultFaultMessage
			 * exception) { System.out.print("Error authenticating with SpiraTest server ("
	...
	public static XMLGregorianCalendar convertDatesJava2Xml(Date date) {
		...
		try {
			...
			// We need to unset the timezone because SpiraTeam is not expected
			// it
